...
void Gamma_pi0_Comp_Plot()
{
 

 
  TFile *fOut = new TFile("pythiaDirectPlots.r03a02rm1.chrg.root", "recreate");
  f_g= new TFile("pythia/pythia23rich.r03a02rm1chrg.d14m1y2017.plots.root");
  f_pi= new TFile("pythia/pythia23rich.r03a02rm1chrg.d14m1y2017.plots.root");
  ...
  h_Datajet_JetPtCorr_g = (TH1F*)*f_g->Get("hGammaDirect");
  h_Datajet_JetPtCorr_pi0 = (TH1F*)*f_pi->Get("hPi0");

  TH1F *h_pi0 =h_Datajet_JetPtCorr_pi0;
  TH1F *h_g =h_Datajet_JetPtCorr_g;
  TString nameStatus = "Charged Jets";
  Double_t Y_max= 1.52;
  ...
  //   c->Range(-7.241379,-2.096007,5.249042,2.699528);
   ...
   //____Y-axis
   h1->GetYaxis()->SetTitle("1/N_{trig} dN_{jets}/(dp_{T,jet}^{reco,ch} d#eta_{jet}) ");
   ...
   Draw_hist(h_g, "PSAME", 2, 2, 4, 1.1 );
   //   Draw_hist(h_pi0, "PSAME", kGreen+4,kGreen+4, 29, 1.6 );
   Draw_hist(h_pi0, "PSAME", 4, 4, 30, 1.6 );
    ...
    Draw_Legend(leg2, 0.54,0.73,0.66,0.97,"p+p 200 GeV",JetRecoLegnd.Data(),"9 < p_{T}^{trig} < 30 GeV/c",nameStatus.Data(),""  );
    
    leg = new TLegend(0.64,0.60,0.86,0.70);
    leg->SetFillColor(0);   leg->SetTextSize(0.03);  
    leg->SetLineColor(0);
    leg->AddEntry(h_g,"Trigger-#gamma_{rich}","p");
    leg->AddEntry(h_pi0,"Trigger-#pi^{0}_{rich}","p");
...
// ------------>Primitives in pad: pad2
   ...
   //_____X-axis
   h3->GetXaxis()->SetTitle("p_{T,jet}^{reco,ch}");
   ...
   h3->GetYaxis()->SetTitle("#pi^{0}_{rich}/#gamma_{rich}");
  ...
  TCanvas *c1 = new TCanvas("c1", "canvas",0,45,821,460);
   ...
   h5->SetLineColor(ci);
   h5->GetXaxis()->SetTitle("p_{T,jet}^{reco,ch}");
   ...
   h5->GetYaxis()->SetTitle("#gamma_{rich}- #pi^{0}_{rich}");
