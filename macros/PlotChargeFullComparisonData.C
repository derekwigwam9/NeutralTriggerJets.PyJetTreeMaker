...
void PlotChargeFullComparisonData()
{
 


  // co-opted old notation:
  //   f_g  ==> File with charge-full comp. for gamma trigger
  //   f_pi ==> File with charge-full comp. for pi0 trigger
  TFile *fOut = new TFile("dataComp.r03a02rm1.correctLabels.d27m1y2017.root", "recreate");
  f_g= new TFile("final/eTtrg30/dataCompG.r03a02rm1.d27m1y2017.root");
  f_pi= new TFile("final/eTtrg30/dataCompP.r03a02rm1.d27m1y2017.root");
  ...
  TH1D *hPi0Ratio;
  if (doComparison) {
    TFile *fPythia = new TFile("pythiaCompR.r05a065rm1.d23m1y2017.root");
    hGamRatio = (TH1D*) fPythia -> Get("hRatioGamma");
    hPi0Ratio = (TH1D*) fPythia -> Get("hRatioPi0");
    hGamRatio -> SetName("hPythiaRatioG");
    hPi0Ratio -> SetName("hPythiaRatioP");
  }
  ...
  h_Datajet_JetPtCorr_gC = (TH1F*)*f_g->Get("hCharge");
  h_Datajet_JetPtCorr_gF = (TH1F*)*f_g->Get("hFull");
  h_Datajet_JetPtCorr_pi0C = (TH1F*)*f_pi->Get("hCharge");
  h_Datajet_JetPtCorr_pi0F = (TH1F*)*f_pi->Get("hFull");

  TH1F *h_pi0C =h_Datajet_JetPtCorr_pi0C;
  TH1F *h_pi0F =h_Datajet_JetPtCorr_pi0F;
  TH1F *h_gC =h_Datajet_JetPtCorr_gC;
  TH1F *h_gF =h_Datajet_JetPtCorr_gF;
  Double_t Y_max= 1.52;

  h_pi0C -> SetNameTitle("hChargePi0", "");
  h_pi0F -> SetNameTitle("hFullPi0", "");
  h_gC   -> SetNameTitle("hChargeGamma", "");
  h_gF   -> SetNameTitle("hFullGamma", "");

  // get ratios
  TH1D *h_RatioG = (TH1D*) f_g -> Get("h_Ratio");
  TH1D *h_RatioP = (TH1D*) f_pi -> Get("h_Ratio");
  h_RatioG -> SetNameTitle("hRatioGamma", "");
  h_RatioP -> SetNameTitle("hRatioPi0", "");
  h_RatioG -> GetFunction("f1") -> Delete();
  h_RatioP -> GetFunction("f1") -> Delete();

  // get differences
  TH1D *h_DiffG = (TH1D*) f_g -> Get("h_Diff");
  TH1D *h_DiffP = (TH1D*) f_pi -> Get("h_Diff");
  h_DiffG -> SetNameTitle("hDiffGamma", "");
  h_DiffP -> SetNameTitle("hDiffPi0", "");
...
*/
  ...
  //   c->Range(-7.241379,-2.096007,5.249042,2.699528);
   ...
   Draw_hist(h_gC, "PSAME", 2, 2, 20, 1.1 );
   Draw_hist(h_gF, "PSAME", 2, 2, 24, 1.1 );
   //   Draw_hist(h_pi0, "PSAME", kGreen+4,kGreen+4, 29, 1.6 );
   Draw_hist(h_pi0C, "PSAME", 4, 4, 29, 1.6 );
   Draw_hist(h_pi0F, "PSAME", 4, 4, 30, 1.6 );
    ...
    Draw_Legend(leg2, 0.54,0.73,0.66,0.97,"p+p 200 GeV",JetRecoLegnd.Data(),"9 < p_{T}^{trig} < 30 GeV/c","#color[2]{STAR Preliminary}",""  );
    
    leg = new TLegend(0.64,0.60,0.86,0.70);
    leg->SetFillColor(0);   leg->SetTextSize(0.03);  
    leg->SetLineColor(0);
    leg->AddEntry(h_gC,"#gamma_{rich}-jet [Charged]","p");
    leg->AddEntry(h_gF,"#gamma_{rich}-jet [Full]","p");
    leg->AddEntry(h_pi0C,"#pi^{0}-jet [Charged]","p");
    leg->AddEntry(h_pi0F,"#pi^{0}-jet [Full]","p");
...
// ------------>Primitives in pad: pad2
   ...
   Int_t pColor;
   if (doComparison) {
     hGamRatio -> SetLineColor(kGreen);
     hGamRatio -> SetMarkerColor(kGreen);
     hPi0Ratio -> SetLineColor(kViolet);
     hPi0Ratio -> SetMarkerColor(kViolet);
     gColor = hGamRatio -> GetMarkerColor();
     pColor = hPi0Ratio -> GetMarkerColor();
   }
   ...
   l->SetLineColor(2);
   l->Draw();
   Draw_hist(h_RatioG, "PESAME", 2,2, 24, 1.2  );
   Draw_hist(h_RatioP, "PESAME", 4,4, 24, 1.2  );
   if (doComparison) Draw_hist(hGamRatio, "PESAME", gColor,gColor, 7, 1.0  );
   if (doComparison) Draw_hist(hPi0Ratio, "PESAME", pColor,pColor, 7, 1.0  );
...
*/
   ...
   leg = new TLegend(0.64,0.60,0.86,0.70);
    ...
    leg->SetLineColor(0);
    leg->AddEntry(h_RatioG, "#gamma_{rich}-jet", "p");
    if (doComparison) leg->AddEntry(hGamRatio, "#gamma_{rich}, Pythia", "p");
    leg->AddEntry(h_RatioP, "#pi^{0}-jet", "p");
    if (doComparison) leg->AddEntry(hPi0Ratio, "#pi^{0}, Pythia", "p"); 
    leg->Draw();
  ...
  TCanvas *c1 = new TCanvas("c1", "canvas",0,45,821,460);
   ...
   l->SetLineColor(1);
   l->Draw();
   Draw_hist(h_DiffG, "PESAME", kOrange+4,kOrange+4, 20, 1.2  );
   Draw_hist(h_DiffP, "PESAME", kViolet+4,kViolet+4, 20, 1.2  );
