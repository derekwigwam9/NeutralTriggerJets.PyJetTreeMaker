...
void MakeDataPlots()
{
  ...
  TFile *fOut = new TFile("dataPlots.r03a02rm1chrg.correctLabels.d27m1y2017.root", "recreate");
  f_g= new TFile("data.r03a02rm1chrg.plots.d26m1y2017.root");
  f_pi= new TFile("data.r03a02rm1chrg.plots.d26m1y2017.root");
  ...
  TF1 *fPythR;
  if (doComparison) {
    TFile *fFuncD = new TFile("final/pythiaDirectPlots.r05a065rm1chrg.d19m1y2017.correctBins.root");
    TFile *fFuncR = new TFile("final/pythiaRichPlots.r05a065rm1chrg.d19m1y2017.correctBins.root");
    fPythD = (TF1*) fFuncD -> Get("f1");
    fPythR = (TF1*) fFuncR -> Get("f1");
    fPythD -> SetName("fPythDirect");
    fPythR -> SetName("fPythRich");
  }
  ...
  h_Datajet_JetPtCorr_g = (TH1F*)*f_g->Get("Gam/hJetPtCorrG");
  ...
  //   c->Range(-7.241379,-2.096007,5.249042,2.699528);
   ...
   Draw_hist(h_g, "PSAME", 2, 2, 8, 1.1 );
   //   Draw_hist(h_pi0, "PSAME", kGreen+4,kGreen+4, 29, 1.6 );
   Draw_hist(h_pi0, "PSAME", 4, 4, 29, 1.6 );
    ...
    leg->SetLineColor(0);
    leg->AddEntry(h_g,"#gamma_{rich}-jets","p");
    leg->AddEntry(h_pi0,"#pi^{0}-jets","p"); 
    leg->Draw();
...
// ------------>Primitives in pad: pad2
   ...
   h3->GetYaxis()->SetTitle("#pi^{0}/#gamma_{rich}");
   ...
   if (doComparison) {
     fPythD -> SetLineColor(kGreen);
     fPythD -> Draw("same");
     fPythR -> SetLineColor(kViolet);
     fPythR -> Draw("same");
   }
...
*/
   ...
   leg = new TLegend(0.64,0.60,0.86,0.70);
    ...
    if (doComparison) {
      leg->AddEntry(fPythD,"Pythia-#gamma_{dir}", "l");
      leg->AddEntry(fPythR,"Pythia-#gamma_{rich}", "l");
      leg->Draw();
    }
  ...
  TCanvas *c1 = new TCanvas("c1", "canvas",0,45,821,460);
   ...
   h5->GetYaxis()->SetTitle("#gamma_{rich}- #pi^{0}");
