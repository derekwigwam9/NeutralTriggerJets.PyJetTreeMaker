...
void PlotChargeFullComparison()
{
  ...
  TFile *fOut = new TFile("dataComp.r03a02rm1.d20m1y2017.root", "recreate");
  f_g= new TFile("final/dataCompG.r03a02rm1.d20m1y2017.root");
  f_pi= new TFile("final/dataCompP.r03a02rm1.d20m1y2017.root");

  // For comparing against data
  //TFile *fFunc = new TFile("pythiaPlots.r05a065rm1.full.root");
  //TF1 *fPyth = (TF1*) fFunc -> Get("f1");
  //fPyth -> SetName("fPyth");
...
*/
  ...
  //   c->Range(-7.241379,-2.096007,5.249042,2.699528);
   ...
   Draw_hist(h_pi0F, "PSAME", 4, 4, 30, 1.6 );
    ...
    Draw_Legend(leg2, 0.54,0.73,0.66,0.97,"p+p 200 GeV",JetRecoLegnd.Data(),"9 < p_{T}^{trig} < 20 GeV/c","",""  );
    
    leg = new TLegend(0.64,0.60,0.86,0.70);
    leg->SetFillColor(0);   leg->SetTextSize(0.03);  
    leg->SetLineColor(0);
    leg->AddEntry(h_gC,"#gamma_{rich}, Charged","p");
    leg->AddEntry(h_gF,"#gamma_{rich}, Full","p");
    leg->AddEntry(h_pi0C,"#pi^{0}_{rich}, Charged","p");
    leg->AddEntry(h_pi0F,"#pi^{0}_{rich}, Full","p");
...
// ------------>Primitives in pad: pad2
   ...
   //_____X-axis
   h3->GetXaxis()->SetTitle("p_{T,jet}^{reco}");
...
*/
   ...
   leg = new TLegend(0.64,0.60,0.86,0.70);
    ...
    leg->SetLineColor(0);
    leg->AddEntry(h_RatioG, "#gamma_{rich}");
    leg->AddEntry(h_RatioP, "#pi^{0}_{rich}");
    //leg->AddEntry(f1,"Pol. fit","l");
    //leg->AddEntry(fPyth,"PYTHIA", "l");
  ...
  TCanvas *c1 = new TCanvas("c1", "canvas",0,45,821,460);
   ...
   h5->SetLineColor(ci);
   h5->GetXaxis()->SetTitle("p_{T,jet}^{reco}");
