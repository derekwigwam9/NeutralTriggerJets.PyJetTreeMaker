...
private:
  ...
  virtual Double_t ApplyCutResponse(const Double_t pTpar);
...
Double_t StFemtoDstMaker::ApplyCutResponse(const Double_t pTpar) {

  // parameters
  const Double_t sigCut = -0.83;
  const Double_t effCut = 0.81;
  const Double_t eff    = effCut * (1.0 - exp(sigCut * pTpar));
  const Double_t pass   = gRandom -> Uniform(0., 1.);
